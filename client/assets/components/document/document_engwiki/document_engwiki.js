(function () {
  angular.module('lucidworksView.components.document_engwiki', ['lucidworksView.services.config',
      'lucidworksView.utils.docs', 'lucidworksView.services.signals'
    ])
    .directive('documentEngwiki', documentEngwiki);

  function documentEngwiki() {
    'ngInject';
    return {
      templateUrl: 'assets/components/document/document_engwiki/document_engwiki.html',
      scope: true,
  ...
  function Controller($log, $scope, DocsHelper, ConfigService, SignalsService,$filter) {
    'ngInject';
    var vm = this;
    ...
    function processDocument(doc) {
      ...
      if(doc.Updated){
        doc.MUpdated_dtFormatted = $filter('date')(doc.Updated,'short');
      }
      if(doc.Created){
